...
public class TestUpdater extends MainClass implements ITestListener {
	...
	// This method will take property from property file and basis on that
	// property it will fetch the list of the test cases.
	...
	// This method will call the getTestCaseList and then perform actions on the
	// test success, the pass id is 1 for success
	@Override
	public void onTestSuccess(ITestResult result) {
		...
		if (update_result) {
			try {
				addTestResults(
				        Integer.parseInt(property.getProperty("testrun_Id")),
			...
			if (!testcases.contains(result.getMethod().getDescription())) {
				System.out.println(
				        "New Test case has been added:- " + result.getMethod());
	...
	// This method will call the getTestCaseList and then perform actions on the
	// test success, the pass id is 5 for failure
	@Override
